...
void shape_factor_calculation(celerity::queue& queue, celerity::buffer<T, 3, compression_tile_type>& tile_points, celerity::buffer<int, 2>& tile_point_count,
	...
	queue.submit([&](celerity::handler& cgh) {
		...
		cgh.parallel_for<class shape_factors>(celerity::nd_range<3>{range, {1, 1, WorkItemsPerTile}}, [=](celerity::nd_item<3> item) {
			...
			auto current_tile = tile_point_acc.decompress_data(
			    item, tile_point_count_acc, tile_point_count_acc[{item.get_global_id(0), item.get_global_id(1)}], {x_min, y_min, 0}, tile_size);

			for(int i = 0; i < num_points_per_runner; i++) {
				size_t idx = i + (item.get_global_id(2) * num_points_per_runner);

				if(tile_point_count_acc[{item.get_global_id(0), item.get_global_id(1)}] < idx) { return; }

				// always use the center tile
				const Point p = current_tile[{item.get_global_id(0), item.get_global_id(1), idx}];
				...
				for(int j = -LocalSearchRadius; j <= LocalSearchRadius; j++) {
					for(int k = -LocalSearchRadius; k <= LocalSearchRadius; k++) {
						if(item.get_global_id(0) + j >= 0 && item.get_global_id(0) + j < item.get_global_range().get(0) && item.get_global_id(1) + k >= 0
						    && item.get_global_id(1) + k < item.get_global_range().get(1)) {
							auto neighborhood = tile_point_acc.decompress_data(item, j, k, tile_point_count_acc,
							    tile_point_count_acc[{item.get_global_id(0) + j, item.get_global_id(1) + k}], {x_min, y_min, 0}, tile_size);
