...
int main(int argc, char* argv[]) {
	...
	celerity::buffer<ShapeFactors, 3> shape_factors({width, height, points_per_tile});

	constexpr int local_search_radius = get_radius(radius, tile_size);

	shape_factor_calculation<local_search_radius, work_items_per_tile, points_per_tile>(queue, tile_points, tile_point_count, shape_factors, radius);
