...
void shape_factor_calculation(celerity::queue& queue, celerity::buffer<T, 3>& tile_points, celerity::buffer<int, 2>& tile_point_count,
    celerity::buffer<U, 3>& shape_factors, const DataTY radius) {
	...
	queue.submit([&](celerity::handler& cgh) {
		...
		cgh.parallel_for<class shape_factors>(celerity::nd_range<3>{range, {1, 1, WorkItemsPerTile}}, [=](celerity::nd_item<3> item) {
			...
			for(int i = 0; i < num_points_per_runner; i++) {
				...
				const Point p = tile_point_acc[{item.get_global_id(0), item.get_global_id(1), idx}];
					...
					for(int k = -LocalSearchRadius; k <= LocalSearchRadius; k++) {
						if(item.get_global_id(0) + j >= 0 && item.get_global_id(0) + j < item.get_global_range().get(0) && item.get_global_id(1) + k >= 0
							...
							for(int l = 0; l < tile_point_count_acc[{item.get_global_id(0) + j, item.get_global_id(1) + k}]; l++) {
								const Point p2 = tile_point_acc[{item.get_global_id(0) + j, item.get_global_id(1) + k, l}];
								const Point p3 = p - p2;
